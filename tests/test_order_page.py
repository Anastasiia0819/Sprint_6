...
class TestOrderPage:
    ...
    def test_order_page(self, driver, order_button_method, telephone_number):
        ...
        with allure.step("клик на заказать (сверху или по середине страницы)"):
        ...
        with allure.step("заполнить данные на странице"):
        #заполнить Имя
        ...
        with allure.step("заплнить данные на странице Про аренду"):
        ...
        # ожидаем модальное окно для подтверждения заказа (кнопка Да)
            ...
            time.sleep(3)
        with allure.step("клик Да - подтверждение заказа"):
        # клик на Да
